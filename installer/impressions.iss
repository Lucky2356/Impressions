...
[InstallDelete]
; Установщик кладёт файлы поверх прежних и никогда ничего не убирает: файл,
; выпавший из поставки, остаётся у человека навсегда. После обновления 1.20.0
; до 1.21.1 в папке лежало 75 МБ такого наследства — kernel_blob.bin от сборок
; до его исключения и sqlite3.dll, вытесненная сборкой с шифрованием. Лишний
; вес полбеды; хуже, что старая библиотека SQLite лежала вплотную к новой.
;
; Каталог ресурсов удаляется целиком: установка выкладывает его заново, и это
; избавляет от такой же уборки за каждым будущим переименованием ресурса.
; Данные при этом не затрагиваются — они лежат в %APPDATA%, а не в {app}.
