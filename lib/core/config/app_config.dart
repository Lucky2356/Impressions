...
class AppConfig {
  ...
  /// Предел распакованного пакета.
  ///
  /// Было 2 ГБ — величина, которая на телефоне не защищает ни от чего: память
  /// кончится задолго до неё, потому что разбор держит содержимое пакета в
  /// оперативной памяти. 256 МБ с запасом покрывают библиотеку с тысячами
  /// записей и сотнями фотографий и при этом остаются подъёмными.
  static const int maxUnpackedBytes = 256 * 1024 * 1024;
  ...
  /// Сколько последних копий, сделанных вручную, хранить.
  ///
  /// Ручные копии живут дольше автоматических — их создают осознанно, перед
  /// чем-то важным. Но и они не должны копиться без предела: каждая копия
  /// содержит всю базу и все изображения.
  static const int manualBackupRetention = 20;
